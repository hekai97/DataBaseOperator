...
//            }
    ...
    public Object[][] ListToObject(ModelFactory factory, String sql) {
        ...
        Object[][] res=null;
        List<E> list=getResult(factory, sql);
        try {
            PreparedStatement preparedStatement=connection.prepareStatement(sql);
            preparedStatement.execute();
            ResultSet resultSet= preparedStatement.executeQuery();
            ResultSetMetaData resultSetMetaData=resultSet.getMetaData();
            ColumnName=getTableColumnName(resultSetMetaData);
            functions=Splitting(factory.getInstance().getClass().getDeclaredMethods(), ColumnName);
        }catch(SQLException e){
            e.printStackTrace();
        }
        for(int i=0;i<ColumnName.length;++i){
        ...
        for(int i=0;i<res.length;++i){
            for(int j=0;j<res[i].length;++j){
                try {
                    res[i][j]=functions[1][j].invoke(list.get(i));
